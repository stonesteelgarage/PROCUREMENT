...
def normalize_vendor_dataframe(df, source_file=""):
    package_col = guess_column(
        df,
        ["package", "pacchetto", "categoria", "lavorazione"]
    )

    supplier_col = guess_column(
        df,
        ["fornitore", "supplier", "vendor", "ragione sociale", "azienda"]
    )
...
def generate_completed_vendor(template_df, threshold=70):
    ...
    for _, row in template_df.iterrows():
        ...
        matches = find_matching_suppliers(
            package_name,
            threshold
        )
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Rispondi solo JSON valido."
                },
                {
                    "role": "user",
                    "content": prompt
...
def real_supplier_scouting(category, area="Italia"):
    ...
    for item in discovered:
        ...
        ai_eval = evaluate_supplier_with_ai(
            category,
            crawled
        )

        matching = int(
            ai_eval.get("matching_percent", 0) or 0
        )

        supplier_name = str(
            ai_eval.get("supplier_name", "")
        ).strip()
        ...
        print("FORNITORE TROVATO:", supplier_name, matching, url)
        ...
        if st.button("Importa nel database"):
            ...
            for file in uploaded_files:
                try:
                    ...
                    st.success(
                        f"{file.name}: {inserted} righe importate"
                    )
